...
maj_tags = full_sense_tags %>% filter(sense_name != "other_meanings") %>%
   group_by(target_child_name, type_sense) %>%
   mutate(n_sense_type_instances = n()) %>% filter(n_sense_type_instances >1) %>% ungroup()

chi_maj_tags = maj_tags %>% filter(speaker_role == "Child",
                                   !sense_name %in% c("wrong_pos", "other_meanings"),
                                   start_age_in_months >= 12,
                                   start_age_in_months < 48)

#what does this do??
chi_maj_tags$downsampled_start_age_in_months = sapply(chi_maj_tags$start_age_in_months, function(x){floor(x /6)*6})

chi_maj_tags$downsampled_start_age_in_months_adjusted = chi_maj_tags$downsampled_start_age_in_months - 12
...
mfs = 	aggregate(sense_name ~ type + downsampled_start_age_in_months + target_child_name, chi_maj_tags, function(x){
		x_tab = table(x)
		return(names(x_tab[order(x_tab)])[1])
})
mfs <- mfs %>% rename("most_common_sense_name" = "sense_name")

# merge most common sense back into chi_maj_tags
chi_maj_tags = merge(chi_maj_tags, mfs)

chi_maj_tags$is_most_common_sense_name = as.numeric(chi_maj_tags$sense_name == chi_maj_tags$most_common_sense_name)

chi_maj_tags$is_not_most_common_sense_name = !chi_maj_tags$is_most_common_sense_name
...
tic()
lm3 = glmer(is_not_most_common_sense_name ~  downsampled_start_age_in_months_adjusted  + (downsampled_start_age_in_months_adjusted | target_child_name), 
            data = subset(maj_tags_ugh, !is.na(is_most_common_sense_name)), 
            family = binomial,
...
sessionInfo()
chi_maj_tags$prediction = sapply(predict(lm3), inv.logit)
mean(chi_maj_tags$prediction)
...
usage_per_noun = aggregate(is_not_most_common_sense_name ~ type, chi_maj_tags, mean) 

names(usage_per_noun)
#pdf('~/Downloads/nounNCMS.pdf')
ggplot(usage_per_noun) + geom_histogram(aes(x= is_not_most_common_sense_name)) + theme_classic() + xlab('Proportion Not Majority Sense')
dev.off()
