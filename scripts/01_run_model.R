...
maj_tags <- full_sense_tags %>%
  ...
  filter(speaker_role == target_speaker_role,
         start_age_in_months >= 12,
         start_age_in_months < 48,
...
maj_tags$downsampled_start_age_in_months_adjusted = maj_tags$downsampled_start_age_in_months - 12
...
model_boot <- bootMer(model, nsim=1000,re.form=NA,  .progress = "txt", FUN = function(x)inv.logit(predict(x, newdata = new_data, re.form=NA)))
